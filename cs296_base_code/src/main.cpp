...
int main(int argc, char** argv)
{
  ...
  //! We create the GLUI user interfacet 
  // create_glui_ui();
  ...
  */
  ...
  // 150 step simulation :-
	...
	b2World* new_b2World = test->get_world();
	
	const b2Profile& p = new_b2World->GetProfile();
	
	float32 single_step_time = 0;
	float32 collision_step_time = 0;
	float32 velocity_update_step_time = 0;
	float32 position_update_step_time = 0;
	float32 loop_time;
	timeval t1, t2;
	...
	float32 time_for_step = settings.hz > 0.0f ? 1.0f / settings.hz : float32(0.0f);
	  for (int i = 0; i < loops; i++){
		new_b2World->Step(time_for_step, settings.velocity_iterations, settings.position_iterations);
		single_step_time += p.step;
		collision_step_time += p.collide;
		velocity_update_step_time += p.solveVelocity;
		position_update_step_time += p.solvePosition;
	  }
  gettimeofday(&t2,NULL);
  loop_time = (t2.tv_sec - t1.tv_sec) * 1000.0;     
  loop_time += (t2.tv_usec - t1.tv_usec) / 1000.0;
  std::printf("%s %i\n", "Number of Iterations: ",loops);
  std::printf("%s %011.5f %s\n", "Average time per step is ", single_step_time/loops, "ms");
  std::printf("%s %011.5f %s\n", "Average time for collisions is ", collision_step_time/loops, "ms");
  std::printf("%s %011.5f %s\n", "Average time for velocity updates is ", velocity_update_step_time/loops, "ms");
  std::printf("%s %011.5f %s\n", "Average time for position updates is ", position_update_step_time/loops, "ms");
  std::printf("%s %011.5f %s\n", "Total loop time is ",loop_time, "ms");
  std::printf("%s\n","This is from the Box2D simulation for CS296 Lab 04. It has been made by Nitin Chandrol from Group 18.");
